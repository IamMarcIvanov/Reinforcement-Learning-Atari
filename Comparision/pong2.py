# this is the one that gave best results
...
class MaxAndSkipEnv(gym.Wrapper):
    ...
    # here he is taking the maximum of each pixel for current and previous frame
    def step(self, action):
        total_reward = 0.0
        done = None
...
class DQN(nn.Module):
    ...
    def _get_conv_out(self, shape):
        # pass an input of 0 (of the same shape as the images) through it and then take product of its shape.
...
TransitionTable = collections.namedtuple('TransitionTable', field_names=['state', 'action', 'reward', 'done', 'new_state'])
...
while True:
    ...
    epsilon = max(EPSILON_FINAL, EPSILON_FINAL + (EPSILON_START - EPSILON_FINAL) * pow(math.e, - curr_frame_idx / EPSILON_DECAY_LAST_FRAME))
